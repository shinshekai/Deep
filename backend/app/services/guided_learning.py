...
class GuidedLearningService:
    ...
    async def start_session(
        ...
        if state.memory_service and device_id:
            try:
                ...
                await state.memory_service.update_profile(
                    device_id,
                    {
        ...
        if state.memory_service and device_id:
            with contextlib.suppress(Exception):
                await state.memory_service.record_agent_outcome(
                    ...
                    strategy="interactive_generation",
                    outcome_quality=0.7,
                    model_used=model_id,
                    device_id=device_id,
                )
    ...
    async def chat(
        ...
        session_data["chat_history"].append(
            {"role": "user", "content": user_message, "point_index": point_index}
        )
        ...
        response = await self.lm_client.stream_chat_completion(
            model=model_id, messages=messages, max_tokens=1000
        )
        ...
        session_data["chat_history"].append(
            {"role": "assistant", "content": answer, "point_index": point_index}
        )
        await asyncio.to_thread(self._save_session, session_id, session_data)
    ...
    async def end_session(
        self, session_id: str, model_id: str = "Qwen3-1.7B-Q4_K_M"
    ) -> dict[str, Any]:
        ...
        chat_history_text = "\n".join(
            [f"{m['role']}: {m['content']}" for m in session_data["chat_history"]]
        )
