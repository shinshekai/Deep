    ...
    try:
        ...
        if _alert_state.vram_critical and not was_crit:
            ...
            ALERT_INFO.labels(type="vram").info(
                {"usage": f"{used_pct:.1f}%", "severity": "critical"}
            )
            ...
            ALERT_INFO.labels(type="vram").info(
                {"usage": f"{used_pct:.1f}%", "severity": "warning"}
            )
...
def _check_error_rate() -> None:
    ...
    _alert_state.high_error_rate = (
        rate >= ERROR_RATE_WARN and _alert_state._error_counts["total"] >= 10
    )
