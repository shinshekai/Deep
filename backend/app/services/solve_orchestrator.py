...
async def run_solve_pipeline(
    ...
    with trace_span("solve.pipeline", {"session_id": session_id, "mode": mode, "kb_name": kb_name}):
        ...
        logger.info(
            f"Solve session {session_id}: score={score}, tier={target_tier}, mode={mode}, vram_free={free_vram_mb:.0f}MB, pages={doc_pages}"
        )
        ...
        try:
            ...
            if state.memory_service:
                if device_id is None:
                    logger.warning(
                        f"Solve session {session_id}: device_id is None; skipping memory write"
                    )
                else:
...
async def _run_dual_loop(
    ...
    for agent_key, agent_label in analysis_steps:
        ...
        if agent_key == "investigate" and kb_name:
            ...
            req = RetrieveRequest(
                query=query, kb_name=kb_name, retrieval_pipeline=retrieval_pipeline, top_k=3
            )
