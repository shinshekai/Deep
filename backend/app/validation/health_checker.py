...
def _check_lm_studio_lifecycle(report: ValidationReport) -> None:
    ...
    if load_fn_match:
        ...
        has_verification = any(kw in body for kw in [
            "verify", "confirm", "status", "assert", "raise",
            "loaded_models", "poll", "check"
        ])
        has_only_logging = (
            "logger." in body
            and "return" in body
    ...
    if unload_fn_match:
        ...
        has_vram_check = any(kw in body for kw in [
            "nvml", "vram", "free", "pynvml", "memory", "verify"
        ])
        report.add(ValidationResult(
            check_id="HLT-002",
            category=CheckCategory.HEALTH,
