...
def test_get_research_status():
    ...
    mock_dr = MagicMock()
    mock_dr.get_status = MagicMock(
        return_value={"status": "COMPLETED", "final_report": "Research complete."}
    )
...
def test_notebook_crud():
    ...
    with (
        ...
        patch.object(
            __import__(
                "app.services.content_creation", fromlist=["NotebookService"]
            ).NotebookService,
            "__init__",
        ...
        # Create notebook
        response = client.post(
            "/api/v1/notebooks", json={"title": "Test Notebook", "description": "A test"}
        )
        assert response.status_code == 200
        data = response.json()
        ...
        # Add note
        response = client.post(
            f"/api/v1/notebooks/{nb_id}/notes", json={"content": "Important note"}
        )
        assert response.status_code == 200
        note = response.json()
...
def test_cowriter_edit():
    ...
    with patch("app.state.lm_client") as mock_lm:
        mock_lm.stream_chat_completion = AsyncMock(return_value={"content": "Shortened version."})

        response = client.post(
            "/api/v1/cowriter/edit",
            json={
